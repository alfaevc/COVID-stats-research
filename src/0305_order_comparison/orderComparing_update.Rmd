...
states = c("FLORIDA", "NEW JERSEY", "KENTUCKY", "TEXAS")
...
#
# states_trend <- ggplot(master_df, aes(Days, newcd_norm_moving, color = color_scale))+
#   geom_path() +
#   geom_point()
...
states_trend <- ggplot() +
  ...
  # geom_path(data=master_df, aes(x=Days, y=newcd_norm_moving,
  #                               group=1,color=color_scale)) +
  scale_fill_manual(values=color_v, name="")+
  # geom_line(data = master_df, aes(x = Days, y = newcd_norm_moving,
  #                                 group=color_scale, color=color_scale),
  #           size = 1, alpha = 0.5) +
  #geom_line(data = master_df, aes(x = Days, y = newcd_norm_moving,
                                  #color = STATE, Date=DATE,
                                  #State=STATE),size = 1, alpha = 0.5) +
  geom_point(data = event_master_df, aes(x = Days,
             ...
             alpha = 1, size=3) +
  geom_point(data=school_opening_df, aes(x = Days,
                                         y = newcd_norm_moving,
                                         shape = Events_mler),
             size=3) +
...
for (i in (1: length(states))){
  ...
  # index of order implementation date
  #state_mask_ind = which(sapply(state_data$Events, check_event, event_order)) 
  first_hit_3 <- which(state_data$Score>=2.5 & state_data$DATE>=may_day)[1]
